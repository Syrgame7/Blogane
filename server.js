...
function saveData() {
    try {
        ...
        if(db.stories.length > 50) db.stories = db.stories.slice(-50); // Keep recent stories
...
for(let i=0; i<130; i++) {
    ...
    if(!db.users.find(u => u.email === email)) {
        ...
        // استخدام صور واقعية من Unsplash
        const gender = ["سارة","نور","ليلى","مريم","فاطمة","هدى","منى","زينب","سلمى","رنا"].includes(f) ? "woman" : "man";
        const avatar = `https://source.unsplash.com/random/200x200/?face,${gender},${i}`;
        
        db.users.push({
            id: Date.now() + i, name: `${f} ${l}`, email: email, password: 'bot',
            // Fallback avatar if unsplash fails/slows, using ui-avatars as backup in frontend logic usually, but here we set a static nice one
            avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${f}${l}${i}`, 
            bio: 'أحب الحياة والتكنولوجيا 🌟', 
            isBot: true, isOnline: true, coins: 50000
        });
    }
}
saveData();
...
// --- Simulation ---
const botMsgs = ["صباح الخير ☀️", "تطبيق رائع", "صورة جميلة", "سبحان الله", "مساء الورد", "مين موجود؟", "جمعة مباركة", "تحياتي", "روعة", "استمر"];
...
setInterval(() => {
    try {
        ...
        if (action < 0.1) { // Story
             const story = { id: Date.now(), author: sender.name, email: sender.email, avatar: sender.avatar, media: `https://picsum.photos/400/800?random=${Date.now()}`, date: new Date().toISOString() };
...
io.on('connection', (socket) => {
    ...
    socket.on('register', (d) => {
        ...
        else { const u = { ...d, id: Date.now(), avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${d.name}`, bio: 'جديد', isOnline: true, coins: 100 }; db.users.push(u); saveData(); socket.emit('auth_success', u); }
    });
    socket.on('login', (d) => {
    ...
    // Features
    socket.on('new_story', (d) => { let u=saveBase64ToFile(d.media, 'story'); if(u){ const s={id:Date.now(), author:d.author, email:d.email, avatar:d.avatar, media:u, date:new Date().toISOString()}; db.stories.push(s); saveData(); io.emit('new_story', s); } });
