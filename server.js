...
io.on('connection', (socket) => {
    ...
    // Game Rewards (Added support for Candy Game)
    socket.on('game_result', ({email, reward, gameName}) => {
    ...
    socket.on('send_private_msg', (d) => { const m = { ...d, id: Date.now(), date: new Date().toISOString() }; db.privateMessages.push(m); saveData(); socket.emit('receive_private_msg', m); sendRecentChats(d.from); const target = db.users.find(u => u.email === d.to); if(target && target.isBot) { setTimeout(async () => { const replyText = await getAIResponse(d.text); const botReply = { id: Date.now(), from: d.to, to: d.from, text: replyText, date: new Date().toISOString() }; db.privateMessages.push(botReply); saveData(); socket.emit('receive_private_msg', botReply); sendRecentChats(d.from); }, 2000); } else if (connectedSockets[d.to]) { io.to(connectedSockets[d.to]).emit('receive_private_msg', m); io.to(connectedSockets[d.to]).emit('notification', {title:'رسالة', body:'لديك رسالة جديدة'}); sendRecentChats(d.to); } });
