...
// --- الذكاء الاصطناعي (نظام المحاولات المتعددة - Robust AI) ---
async function getAIResponse(prompt) {
    if (!process.env.GEMINI_API_KEY) return "عذراً، مفتاح AI غير مضبوط.";
    ...
    return "آسف، واجهت مشكلة تقنية في الاتصال بجوجل. حاول لاحقاً! 🤖";
}
...
const botBios = ["أحب الحياة 🌸", "مبرمج ومصمم 💻", "عاشق القهوة ☕", "طالب علم 📚", "مهندس", "طبيب", "سبحان الله", "مصور فوتوغرافي 📸"];

for(let i=0; i<80; i++) {
    const email = `user_${i}@blogane.com`;
    if(!db.users.find(u => u.email === email)) {
        const fname = botFirstNames[Math.floor(Math.random() * botFirstNames.length)];
        const lname = botLastNames[Math.floor(Math.random() * botLastNames.length)];
        db.users.push({
            ...
            bio: botBios[Math.floor(Math.random() * botBios.length)], isBot: true, isOnline: true
        });
    }
}
saveData();

// محرك التفاعل (صور ونصوص)
const botPostsText = ["صباح الخير 🌹", "جمعة مباركة", "مساء الورد", "مين فاضي نتكلم؟", "صورة جميلة", "سبحان الله", "الحمد لله", "تصبحون على خير", "الجو رائع اليوم"];
const botImages = [
    ...
    try {
        ...
        if (action < 0.15) { // نشر
            const newPost = {
                ...
                content: botPostsText[Math.floor(Math.random() * botPostsText.length)], 
                media: botImages[Math.floor(Math.random() * botImages.length)],
        ...
        else if (action < 0.75 && db.posts.length > 0) { // تعليق
            const p = db.posts[Math.floor(Math.random() * db.posts.length)];
            if(p) {
                const c = { id: Date.now(), text: "منور 🔥", userEmail: botUser.email, userName: botUser.name, userAvatar: botUser.avatar };
...
io.on('connection', (socket) => {
    ...
    socket.on('register', (d) => { if (db.users.find(u => u.email === d.email)) socket.emit('auth_error', 'البريد مسجل'); else { const u = { ...d, id: Date.now(), avatar: `https://ui-avatars.com/api/?name=${d.name}&background=random`, bio: 'مستخدم جديد', isOnline: true }; db.users.push(u); saveData(); socket.emit('auth_success', u); } });
    socket.on('login', (d) => { const u = db.users.find(u => u.email === d.email && u.password === d.password); if (u) { connectedSockets[u.email] = socket.id; u.isOnline = true; saveData(); socket.emit('auth_success', u); socket.emit('init_data', { groups: db.groups, pages: db.pages, reels: db.reels, globalMessages: db.globalMessages }); socket.emit('load_posts', (db.posts||[]).filter(p => p.context === 'general')); updateFriendsList(u.email); checkFriendRequests(u.email); } else { socket.emit('auth_error', 'بيانات خاطئة'); } });
    ...
    socket.on('send_private_msg', (d) => {
        ...
        if(target && target.isBot) {
            setTimeout(async () => {
                const replyText = await getAIResponse(d.text); // البوت يستخدم نفس الذكاء
    ...
    function updateFriendsList(email) { const fs=db.friendships.filter(f=>f.user1===email||f.user2===email); const es=fs.map(f=>f.user1===email?f.user2:f.user1); const fd=db.users.filter(u=>es.includes(u.email)).map(u=>({name:u.name, email:u.email, avatar:u.avatar, isOnline:!!connectedSockets[u.email]||u.isBot})); if(connectedSockets[email])io.to(connectedSockets[email]).emit('update_friends', fd); }
    ...
    socket.on('get_profile_info', (e)=>{const u=db.users.find(x=>x.email===e); if(u){const fs=db.friendships.filter(f=>f.user1===e||f.user2===e); const fe=fs.map(f=>f.user1===e?f.user2:f.user1); const fr=db.users.filter(x=>fe.includes(x.email)).map(x=>({name:x.name, avatar:x.avatar, email:x.email})); socket.emit('open_profile_view', {user:u, posts:(db.posts||[]).filter(p=>p.email===e), friends:fr});}});
