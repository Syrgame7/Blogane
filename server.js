...
for(let i=0; i<130; i++) {
    ...
    if(!db.users.find(u => u.email === email)) {
        ...
        // صور كرتونية عالية الجودة
        const avatar = `https://api.dicebear.com/7.x/avataaars/svg?seed=${f}${l}${i}&backgroundColor=b6e3f4,c0aede,d1d4f9`;
...
}, 4000); // كل 4 ثواني نشاط

io.on('connection', (socket) => {
    socket.on('register', (d) => {
        if (db.users.find(u => u.email === d.email)) socket.emit('auth_error', 'البريد مسجل');
        else { const u = { ...d, id: Date.now(), avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${d.name}`, bio: 'جديد', isOnline: true, coins: 100, verified: false }; db.users.push(u); saveData(); socket.emit('auth_success', u); }
    });
    socket.on('login', (d) => {
