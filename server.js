...
async function getAIResponse(prompt) {
    if (!process.env.GEMINI_API_KEY) return "أنا مساعدك الذكي، كيف أخدمك؟";
    try {
...
const bios = ["أحب الحياة 🌸", "مبرمج", "فنان", "طبيب", "مهندس", "طالب", "محب للخير", "عاشق للسفر"];
...
const botMsgs = ["صباح الخير", "منشور جديد", "صورة جميلة", "سبحان الله", "يوم سعيد", "تطبيق رائع", "جمعة مباركة", "مساء النور"];
...
io.on('connection', (socket) => {
    ...
    socket.on('register', (d) => {
        if (db.users.find(u => u.email === d.email)) socket.emit('auth_error', 'هذا البريد مسجل مسبقاً!');
        else { 
            const u = { ...d, id: Date.now(), avatar: `https://ui-avatars.com/api/?name=${d.name}&background=random`, bio: 'مستخدم جديد', isOnline: true, coins: 100 }; 
            db.users.push(u); saveData(); 
    ...
    socket.on('search_users', (query) => {
        if(!query) return;
        const results = db.users.filter(u => u.name.toLowerCase().includes(query.toLowerCase())).slice(0, 10).map(u => ({
            name: u.name, email: u.email, avatar: u.avatar, isBot: u.isBot
        }));
