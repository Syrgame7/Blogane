...
function saveData() {
    try {
        // تنظيف تلقائي للحفاظ على السرعة
...
const bios = ["أحب الحياة 🌸", "مصور فوتوغرافي 📸", "مبرمج ومطور 💻", "عاشق القهوة ☕", "قارئ نهم 📚", "مهندس معماري 🏗️", "طبيب المستقبل 🩺"];
...
for(let i=0; i<130; i++) {
    ...
    if(!db.users.find(u => u.email === email)) {
        ...
        const gender = ["سارة","نور","ليلى","مريم","فاطمة","هدى","منى","زينب","سلمى","رنا"].includes(names[Math.floor(Math.random()*names.length)]) ? "women" : "men";
        db.users.push({
            id: Date.now() + i, name: n, email: email, password: 'bot',
            // استخدام صور رمزية ملونة ومميزة
            avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${n}&backgroundColor=b6e3f4,c0aede,d1d4f9`,
...
// --- Simulation (Ticker, Posts, Stories) ---
...
io.on('connection', (socket) => {
    ...
    function sendRecentChats(email) {
        ...
        for(const m of allMsgs) {
            ...
            if(other && !chats.has(other)) {
                ...
                if(u) recentUsers.push({ name: u.name, email: u.email, avatar: u.avatar, isBot: u.isBot, verified: u.verified });
            }
    ...
    socket.on('get_profile_info', (e) => { 
        ...
        } else { socket.emit('notification', {title:'خطأ', body:'المستخدم غير موجود'}); }
    });
