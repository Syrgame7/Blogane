...
function saveData() {
    try {
        // تنظيف دوري (للحفاظ على سرعة السيرفر)
...
const botBios = ["أحب الحياة 🌸", "مبرمج هاوي 💻", "مصور فوتوغراف 📸", "عاشق القهوة ☕", "طالب علم 📚", "مهندس معماري 🏗️", "طبيب المستقبل 🩺", "كاتب خواطر ✍️"];
...
setInterval(() => {
    try {
        ...
        // نختار بوت عشوائي من قاعدة البيانات (الذين هم بوتات فقط)
...
io.on('connection', (socket) => {
    ...
    socket.on('send_ai_msg', async (text) => {
        // إشعار بأن البوت يكتب... (اختياري في الواجهة)
        const reply = await askGemini(text);
        socket.emit('receive_ai_msg', { text: reply });
    });

    // Friends Logic (With Auto-Accept Bot)
    ...
    socket.on('send_private_msg', (d) => {
        ...
        if(target && target.isBot) {
            setTimeout(async () => {
                // البوت يستخدم Gemini أيضاً للرد عليك في الخاص!
                const replyText = await askGemini(d.text);
