...
const bioList = ["أحب الحياة ✨", "صانع محتوى 🎥", "مبرمج 💻", "مسافر ✈️", "طبيب 🩺", "مهندس 🏗️", "عاشق القهوة ☕"];
...
for(let i=0; i<130; i++) {
    ...
    if(!db.users.find(u => u.email === email)) {
        ...
        const gender = ["سارة","نور","ليلى","مريم","فاطمة","هدى","منى","زينب","سلمى","رنا"].includes(n.split(' ')[0]) ? "women" : "men";
        db.users.push({
            id: Date.now() + i, name: n, email: email, password: 'bot',
            avatar: `https://randomuser.me/api/portraits/${gender}/${i % 99}.jpg`, 
...
setInterval(() => {
    try {
        ...
        } else if (action < 0.4) { // Post (General Feed)
            const p = { 
                ...
                context:'general', contextId:null, verified: true 
            };
            db.posts.unshift(p); 
            io.emit('receive_post', p); // Broadcast to ALL
        }
        saveData();
    } catch(e) {}
}, 4000);
...
io.on('connection', (socket) => {
    ...
    socket.on('get_profile_info', (email) => { 
        ...
        if(u) { 
            ...
            const userPosts = db.posts.filter(p => p.email === email); // Filter posts for this user
    ...
    // Standard Handlers
    socket.on('register', (d) => { if(db.users.find(u=>u.email===d.email))socket.emit('auth_error','مسجل'); else {const u={...d,id:Date.now(),avatar:`https://api.dicebear.com/7.x/avataaars/svg?seed=${d.name}`,bio:'جديد',isOnline:true,coins:100,verified:false}; db.users.push(u); saveData(); socket.emit('auth_success',u);} });
    socket.on('login', (d) => { const u=db.users.find(x=>x.email===d.email && x.password===d.password); if(u){connectedSockets[u.email]=socket.id; u.isOnline=true; if(u.coins==undefined)u.coins=100; saveData(); socket.emit('auth_success',u); socket.emit('init_data',{groups:db.groups,pages:db.pages,reels:db.reels,globalMessages:db.globalMessages,friendRequests:db.friendRequests,stories:db.stories}); socket.emit('load_posts', db.posts.filter(p=>p.context==='general')); const f=db.friendships.filter(x=>x.user1===u.email||x.user2===u.email).map(x=>x.user1===u.email?x.user2:x.user1); const fd=db.users.filter(x=>f.includes(x.email)).map(x=>({name:x.name,email:x.email,avatar:x.avatar,isOnline:!!connectedSockets[x.email]||x.isBot,verified:x.verified})); socket.emit('update_friends',fd); sendRecentChats(u.email); } else socket.emit('auth_error','خطأ'); });

    function sendRecentChats(email) { const msgs=[...db.privateMessages].reverse(); const chats=new Set(); const users=[]; for(const m of msgs){ const o=m.from===email?m.to:(m.to===email?m.from:null); if(o&&!chats.has(o)){chats.add(o); const u=db.users.find(x=>x.email===o); if(u)users.push({name:u.name,email:u.email,avatar:u.avatar,isBot:u.isBot,verified:u.verified});} if(users.length>=15)break;} if(connectedSockets[email])io.to(connectedSockets[email]).emit('update_recent_chats',users); }
