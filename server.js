...
io.on('connection', (socket) => {
    ...
    // Power Ups (Candy Game) - Deduct coins
    socket.on('buy_power', ({email, cost}) => {
        const u = db.users.find(x => x.email === email);
        if(u && u.coins >= cost) {
            u.coins -= cost;
            saveData();
    ...
    socket.on('send_private_msg', (d) => {
        ...
        if(target && target.isBot) { setTimeout(async()=>{ const r=await getAIResponse(d.text); db.privateMessages.push({id:Date.now(),from:d.to,to:d.from,text:r,date:new Date().toISOString()}); saveData(); socket.emit('receive_private_msg',{from:d.to,text:r}); sendRecentChats(d.from); }, 2000); }
