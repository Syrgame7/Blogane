...
io.on('connection', (socket) => {
    ...
    socket.on('login', (d) => { const u = db.users.find(u => u.email === d.email && u.password === d.password); if (u) { connectedSockets[u.email] = socket.id; u.isOnline = true; if(u.coins===undefined)u.coins=100; saveData(); socket.emit('auth_success', u); socket.emit('init_data', { groups: db.groups, pages: db.pages, reels: db.reels, globalMessages: db.globalMessages, friendRequests: db.friendRequests }); socket.emit('load_posts', (db.posts||[]).filter(p => p.context === 'general')); updateFriendsList(u.email); checkFriendRequests(u.email); sendRecentChats(u.email); } else { socket.emit('auth_error', 'بيانات خاطئة'); } });

    // Profile Fix: Send SPECIFIC user data, not generic
    socket.on('get_profile_info', (e) => { 
        ...
        if(u) { 
            ...
            socket.emit('profile_error', 'المستخدم غير موجود');
        }
    });
    ...
    socket.on('send_private_msg', (d) => {
        ...
        if(target && target.isBot) {
            setTimeout(async () => {
                ...
                socket.emit('receive_private_msg', botReply); sendRecentChats(d.from);
            }, 2000);
        } else if (connectedSockets[d.to]) {
            io.to(connectedSockets[d.to]).emit('receive_private_msg', m);
            io.to(connectedSockets[d.to]).emit('notification', {title:'رسالة', body:'لديك رسالة جديدة'});
            sendRecentChats(d.to);
        }
    });
    ...
    socket.on('claim_bonus', (email) => { const u=db.users.find(x=>x.email===email); if(u){u.coins+=50; saveData(); socket.emit('update_coins', u.coins);} });
