...
// --- Bot Simulation (Wisdom & Quotes) ---
const botQuotes = [
    "لا تيأس، فكل ليل يليه نهار مشرق.", 
    "الصبر مفتاح الفرج.", 
    "النجاح ليس صدفة، بل هو نتيجة اجتهاد.",
...
setInterval(() => {
    try {
        ...
        } else if (action < 0.3) { // Post Quote
            const p = { id: Date.now(), author: sender.name, email: sender.email, avatar: sender.avatar, content: botQuotes[Math.floor(Math.random()*botQuotes.length)], media: null, likes: [], comments: [], date: new Date().toISOString(), context:'general', contextId:null };
...
io.on('connection', (socket) => {
    socket.on('register', (d) => { if (db.users.find(u => u.email === d.email)) socket.emit('auth_error', 'البريد مسجل'); else { const u = { ...d, id: Date.now(), avatar: `https://ui-avatars.com/api/?name=${d.name}&background=random`, bio: 'جديد', isOnline: true, coins: 100 }; db.users.push(u); saveData(); socket.emit('auth_success', u); } });
    socket.on('login', (d) => { const u = db.users.find(u => u.email === d.email && u.password === d.password); if (u) { connectedSockets[u.email] = socket.id; u.isOnline = true; if(u.coins===undefined)u.coins=100; saveData(); socket.emit('auth_success', u); socket.emit('init_data', { groups: db.groups, pages: db.pages, reels: db.reels, globalMessages: db.globalMessages, friendRequests: db.friendRequests }); socket.emit('load_posts', (db.posts||[]).filter(p => p.context === 'general')); const fs=db.friendships.filter(f=>f.user1===u.email||f.user2===u.email); const fe=fs.map(f=>f.user1===u.email?f.user2:f.user1); const fd=db.users.filter(x=>fe.includes(x.email)).map(x=>({name:x.name,email:x.email,avatar:x.avatar,isOnline:!!connectedSockets[x.email]||x.isBot})); socket.emit('update_friends', fd); sendRecentChats(u.email); } else { socket.emit('auth_error', 'بيانات خاطئة'); } });

    function sendRecentChats(email) { const allMsgs = [...db.privateMessages].reverse(); const chats = new Set(); const recentUsers = []; for(const m of allMsgs) { const other = m.from === email ? m.to : (m.to === email ? m.from : null); if(other && !chats.has(other)) { chats.add(other); const u = db.users.find(x => x.email === other); if(u) recentUsers.push({ name: u.name, email: u.email, avatar: u.avatar, isBot: u.isBot }); } if(recentUsers.length >= 15) break; } if(connectedSockets[email]) io.to(connectedSockets[email]).emit('update_recent_chats', recentUsers); }
    ...
    socket.on('send_private_msg', (d) => { const m = { ...d, id: Date.now(), date: new Date().toISOString() }; db.privateMessages.push(m); saveData(); socket.emit('receive_private_msg', m); sendRecentChats(d.from); const t = db.users.find(u => u.email === d.to); if(t && t.isBot) { setTimeout(async()=>{const r=await getAIResponse(d.text); db.privateMessages.push({id:Date.now(),from:d.to,to:d.from,text:r,date:new Date().toISOString()}); saveData(); socket.emit('receive_private_msg',{from:d.to,text:r}); sendRecentChats(d.from);},2000); } else if(connectedSockets[d.to]){ io.to(connectedSockets[d.to]).emit('receive_private_msg',m); io.to(connectedSockets[d.to]).emit('notification',{title:'رسالة',body:'لديك رسالة جديدة'}); sendRecentChats(d.to); } });
