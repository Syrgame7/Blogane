...
async function getAIResponse(prompt) {
    if (!process.env.GEMINI_API_KEY) return "أهلاً! أنا مساعدك الذكي.";
    try {
...
for(let i=0; i<130; i++) {
    ...
    if(!db.users.find(u => u.email === email)) {
        ...
        db.users.push({
            ...
            bio: 'Bot AI 🤖', isBot: true, isOnline: true, coins: 5000
        });
    }
}
saveData();
...
io.on('connection', (socket) => {
    ...
    socket.on('login', (d) => {
        ...
        if (u) {
            ...
            const fEmails = fs.map(f => f.user1 === u.email ? f.user2 : f.user1);
            const fData = db.users.filter(x => fEmails.includes(x.email)).map(x => ({name:x.name, email:x.email, avatar:x.avatar, isOnline:!!connectedSockets[x.email]||x.isBot}));
            socket.emit('update_friends', fData);
