...
for(let i=0; i<130; i++) {
    ...
    if(!db.users.find(u => u.email === email)) {
        const n = names[Math.floor(Math.random()*names.length)] + " Bot";
        db.users.push({
            id: Date.now() + i, name: n, email: email, password: 'bot',
            avatar: `https://ui-avatars.com/api/?name=${n.replace(' ','+')}&background=random&color=fff`,
            bio: 'AI User 🤖', isBot: true, isOnline: true, coins: 50000,
            xp: Math.floor(Math.random() * 5000), level: Math.floor(Math.random() * 20) + 1, verified: true
        });
    }
}
saveData();
...
setInterval(() => {
    try {
        ...
        if (action < 0.1) { // New Story
            const story = { id: Date.now(), author: sender.name, email: sender.email, avatar: sender.avatar, media: `https://picsum.photos/200/300?random=${Date.now()}`, date: new Date().toISOString() };
            db.stories.push(story);
            if(db.stories.length > 50) db.stories.shift();
            io.emit('new_story', story);
        } else if (action < 0.2) { // Gift
...
io.on('connection', (socket) => {
    ...
    socket.on('register', (d) => {
        if (db.users.find(u => u.email === d.email)) socket.emit('auth_error', 'البريد مسجل');
            ...
            const u = { ...d, id: Date.now(), avatar: `https://ui-avatars.com/api/?name=${d.name}&background=random`, bio: 'جديد', isOnline: true, coins: 100, xp: 0, level: 1, verified: false }; 
    ...
    socket.on('login', (d) => {
        ...
        if (u) {
            connectedSockets[u.email] = socket.id; u.isOnline = true; 
            if(u.coins===undefined)u.coins=100; if(u.xp===undefined)u.xp=0; if(u.level===undefined)u.level=1;
            saveData();
    ...
    function addXP(email, amount) {
        ...
        if(u) {
            u.xp += amount;
            if(u.xp >= u.level * 100) { u.level++; u.xp = 0; socket.emit('notification', {title:'Level Up!', body:`وصلت للمستوى ${u.level}`}); }
            saveData();
        }
    }
    ...
    // Stories
    socket.on('add_story', (d) => {
        let u=saveBase64ToFile(d.media,'story');
        if(u) {
            const s = { id: Date.now(), author: d.author, email: d.email, avatar: d.avatar, media: u, date: new Date().toISOString() };
            db.stories.push(s); saveData(); io.emit('new_story', s);
        }
    });

    // Game Rewards
    socket.on('game_result', ({email, reward}) => { const u = db.users.find(x => x.email === email); if(u) { u.coins += reward; addXP(email, 10); saveData(); socket.emit('update_coins', u.coins); } });
    socket.on('claim_bonus', (email) => { const u = db.users.find(x => x.email === email); if(u) { u.coins += 50; addXP(email, 5); saveData(); socket.emit('update_coins', u.coins); socket.emit('notification', {title:'مكافأة!', body:'50 كوينز!'}); } });
    ...
    socket.on('new_post', (d) => { let u=d.media&&d.media.startsWith('data:')?saveBase64ToFile(d.media,'post'):null; const user=db.users.find(x=>x.email===d.email); const p={...d,id:Date.now(),media:u,likes:[],comments:[],date:new Date().toISOString(), verified: user?user.verified:false}; db.posts.unshift(p); if(user){user.coins+=10;addXP(d.email, 20);socket.emit('update_coins',user.coins);} saveData(); io.emit('receive_post', p); socket.emit('upload_complete'); });
    socket.on('toggle_like', ({id, type, userEmail}) => { let x=db.posts.find(i=>i.id==id); if(x){ if(x.likes.includes(userEmail))x.likes=x.likes.filter(e=>e!==userEmail); else {x.likes.push(userEmail); addXP(userEmail, 2);} saveData(); io.emit('update_likes', {id, likes:x.likes}); } });
    socket.on('add_comment', (d) => { const p=db.posts.find(x=>x.id==d.postId); if(p){ p.comments.push({id:Date.now(), ...d}); addXP(d.userEmail, 5); saveData(); io.emit('update_comments', {postId:d.postId, comments:p.comments}); } });
    ...
    socket.on('send_private_msg', (d) => {
        ...
        if(t && t.isBot) { setTimeout(async()=>{const r=await getAIResponse(d.text); db.privateMessages.push({id:Date.now(),from:d.to,to:d.from,text:r,date:new Date().toISOString()}); saveData(); socket.emit('receive_private_msg',{from:d.to,text:r}); sendRecentChats(d.from);}, 2000); }
