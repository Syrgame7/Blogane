...
async function getAIResponse(prompt) {
    if (!process.env.GEMINI_API_KEY) return "أهلاً بك! أنا مساعدك الذكي.";
    try {
...
const bios = ["أحب التكنولوجيا 💻", "مصور فوتوغرافي 📸", "مهندس برمجيات 🏗️", "طبيب 🩺", "قارئ نهم 📚", "عاشق السفر ✈️", "أبحث عن أصدقاء جدد 🤝"];
...
for(let i=0; i<130; i++) {
    ...
    if(!db.users.find(u => u.email === email)) {
        ...
        db.users.push({
            ...
            avatar: `https://ui-avatars.com/api/?name=${n.replace(' ','+')}&background=random&color=fff&size=128`,
            bio: bios[Math.floor(Math.random()*bios.length)], 
            isBot: true, isOnline: true, coins: 5000
        });
    }
}
saveData();
...
const botMsgs = ["صباح الخير يا شباب", "منشور رائع!", "مين يلعب؟", "صورة جميلة", "سبحان الله", "تطبيق ممتاز", "مساء الورد", "جمعة مباركة", "تحياتي للجميع", "روعة"];
...
setInterval(() => {
    try {
        ...
        } else if (action < 0.5 && db.posts.length > 0) { // لايك على منشور عشوائي
             const p = db.posts[Math.floor(Math.random()*db.posts.length)];
             if(p && !p.likes.includes(bot.email)) { p.likes.push(bot.email); io.emit('update_likes', {id:p.id, type:'post', likes:p.likes}); }
        } else if (action < 0.6) { // هدية في الشريط
...
io.on('connection', (socket) => {
    ...
    socket.on('register', (d) => {
        ...
        else { const u = { ...d, id: Date.now(), avatar: `https://ui-avatars.com/api/?name=${d.name}&background=random`, bio: 'مستخدم جديد', isOnline: true, coins: 100 }; db.users.push(u); saveData(); socket.emit('auth_success', u); }
    });
    ...
    // --- Friend Requests (Bot Auto Accept) ---
            ...
            if(t && t.isBot) {
                ...
                setTimeout(() => {
                    ...
                    if(connectedSockets[d.from]) {
                         ...
                         io.to(connectedSockets[d.from]).emit('notification', {title:'تم القبول', body:`${t.name} قبل صداقتك`});
                    }
                }, 2000);
            } else {
    ...
    socket.on('send_private_msg', (d) => {
        ...
        else if (connectedSockets[d.to]) { io.to(connectedSockets[d.to]).emit('receive_private_msg', m); io.to(connectedSockets[d.to]).emit('notification', {title:'رسالة', body:'لديك رسالة جديدة'}); }
    });
    ...
    socket.on('send_gift', (d) => {
        ...
        if (s && s.coins >= d.cost) {
            ...
            if (connectedSockets[d.to]) { io.to(connectedSockets[d.to]).emit('receive_private_msg', msg); io.to(connectedSockets[d.to]).emit('update_coins', r.coins); }
    ...
    socket.on('update_profile', (d) => { const i=db.users.findIndex(u=>u.email===d.email); if(i!==-1){ db.users[i].name=d.name; db.users[i].bio=d.bio; if(d.avatar&&d.avatar.startsWith('data:'))db.users[i].avatar=saveBase64ToFile(d.avatar,'avatar'); saveData(); const u=db.users[i]; db.posts.forEach(p=>{if(p.email===u.email){p.author=u.name;p.avatar=u.avatar}}); socket.emit('profile_updated_success', u); } });
